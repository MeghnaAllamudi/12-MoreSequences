...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window, circle, 6, 'green')
    window.continue_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(slices)):
        line = rg.Line(circle.center,slices[k])
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(350, 200), 150)
    circle.outline_thickness = 3
    polygon(window, circle, 4, 'purple', 10)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    """
    circle.attach_to(window)
    polygon1 = generate_points_on_circle(circle, number_of_segments)
    for k in range(len(polygon1)):
        line = rg.Line(polygon1[k-1], polygon1[k])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    """
    circle.attach_to(window)
    polygon1 = generate_points_on_circle(circle, number_of_lines)
    for k in range(len(polygon1)):
        start = polygon1[k]
